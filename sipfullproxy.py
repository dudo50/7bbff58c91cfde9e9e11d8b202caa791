...
class UDPHandler(socketserver.BaseRequestHandler):   
    ...
    def processRegister(self):
        ...
        if expires == 0:
            if fromm in registrar:
                del registrar[fromm]
                self.sendResponse("200 Vsicko vporadku")
                return
        else:
            now = int(time.time())
    ...
    def processInvite(self):
        ...
        if len(destination) > 0:
            if destination in registrar and self.checkValidity(destination):
                ...
                showtime()
                logging.info("Hovor od "+ origin + " pre " + destination + "\n")
            else:
    ...
    def processNonInvite(self, number):
        origin = self.getOrigin()
        ...
        if len(destination) > 0:
            if destination in registrar and self.checkValidity(destination):
                ...
                showtime()
                if(number == 1):
                    logging.info("Ziadost o zrusenie hovoru medzi " + destination + " a " +  origin + "\n" )
                elif(number == 2):
                    logging.info("Hovor medzi " + destination + " a " +  origin + " zruseny\n" ) 
            else:
                self.sendResponse("406 Nepovoleny pristup")
        else:
            self.sendResponse("500 Interna chyba serveru")
    ...
    def processRequest(self):
        ...
        if len(self.data) > 0:
            ...
            if request_uri== "SIP/2.0 603 Decline":
                logging.info("-----------------------------")
                logging.info(" Pouzivatel je zaneprazdneny ")
                logging.info("-----------------------------")
            if rx_register.search(request_uri):
                self.processRegister()
            elif rx_invite.search(request_uri):
                logging.info("---------------------------")
                logging.info(" Ziadost o hovor obdrzana: ")
                logging.info("---------------------------")
                self.processInvite()
            elif rx_ack.search(request_uri):
                logging.info("---------------------------")
                logging.info(" ACK prijaty pouzivatelom: ")
                logging.info("---------------------------")
                self.processAck()
            elif rx_bye.search(request_uri):
                logging.info("-------------------------------------")
                logging.info(" Ziadost o ukoncenie hovoru obdrzana ")
                logging.info("-------------------------------------")
                self.processNonInvite(1)
            elif rx_cancel.search(request_uri):
                logging.info("-------------------------------------")
                logging.info(" Hovor zruseny osobou ktora vytacala ")
                logging.info("-------------------------------------")
                self.processNonInvite(2)
            elif rx_options.search(request_uri):
                self.processNonInvite(3)
            elif rx_info.search(request_uri):
                self.processNonInvite(4)
            elif rx_message.search(request_uri):
                self.processNonInvite(5)
            elif rx_refer.search(request_uri):
                self.processNonInvite(6)
            elif rx_prack.search(request_uri):
                self.processNonInvite(7)
            elif rx_update.search(request_uri):
                self.processNonInvite(8)
